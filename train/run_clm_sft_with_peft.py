...
from iscasmodel.core import FLOAT16
...
class SavePeftModelCallback(transformers.TrainerCallback):
    def __init__(self):
        ...
        self.last_saved_path = None  # 用于记录最后一次保存的路径
    ...
    def on_epoch_end(self, args, state, control, **kwargs):
        if self.epoch_start_time is not None:
            ...
            print(f"Epoch {state.epoch} duration: {epoch_duration:.2f} seconds")
        ...
        print(f"Step {state.global_step}: GPU Memory Usage")
        for i in range(torch.cuda.device_count()):
            print(f"Device {i}: {torch.cuda.memory_allocated(i) / 1e9:.2f} GB Allocated")
            print(f"Device {i}: {torch.cuda.memory_reserved(i) / 1e9:.2f} GB Reserved")
        self.save_model(args, state, kwargs)
        return control
...
def prepare_model_for_kbit_training(model, use_gradient_checkpointing=True):
    r"""
    This method wraps the entire protocol for preparing a model before running a train. This includes:
...
class DataTrainingArguments:
    """
    Arguments pertaining to what data we are going to input our model for train and eval.
    """
    ...
    train_file: Optional[str] = field(default=None, metadata={"help": "The input train data file (a text file)."})
    ...
    overwrite_cache: bool = field(
        default=False, metadata={"help": "Overwrite the cached train and evaluation sets"}
    )
    ...
    max_seq_length: Optional[int] = field(default=1024,
                                          metadata={"help": "The longest length of train or validation data"})
...
def main():
    ...
    logger.warning(
        ...
        + f"distributed train: {bool(training_args.local_rank != -1)}, 16-bits train: {training_args.fp16 or training_args.bf16}"
    )
    ...
    if os.path.isdir(training_args.output_dir) and training_args.do_train and not training_args.overwrite_output_dir:
        ...
        if last_checkpoint is None and len(os.listdir(training_args.output_dir)) > 0:
            ...
            logger.info(
                f"Checkpoint detected, resuming train at {last_checkpoint}. To avoid this behavior, change "
    ...
    if training_args.do_train:
        with training_args.main_process_first(desc="loading and tokenization"):
            ...
            print('process train dataset successfully')
    ...
    model_utils = ModelUtils()
    total_batch_size = model_utils.model_memory_usage(model, 2, FLOAT16)
